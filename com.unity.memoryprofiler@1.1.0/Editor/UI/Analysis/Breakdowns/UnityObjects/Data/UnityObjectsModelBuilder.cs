...
namespace Unity.MemoryProfiler.Editor.UI
{
    ...
    class UnityObjectsModelBuilder
    {
            ...
            out MemorySize totalMemoryInSnapshot)
        {
            ...
            // rby Filter
            var rBySearcher = new ReferenceSearcher();
            ...
            foreach (var obj in nativeObject2Size)
            {
                ...
                if (args.SearchStringFilter != null && args.SearchStringFilter.FilterValue.StartsWith("rby:"))
                {
                    var od = ObjectData.FromNativeObjectIndex(snapshot, (int)obj.Key.Index);
                    var filterValue = args.SearchStringFilter.FilterValue.Substring(4);
                    bool b = rBySearcher.CheckReferencedBy(snapshot, od, data =>
                    {
                        var typeName = data.GenerateTypeName(snapshot);
                        return filterValue.Equals(typeName);
                    });
                    
                    if (!b)
                        continue;
                }
                else if (args.SearchStringFilter != null)
                {
